...
class CombineController extends Controller
{
     ...
     public function Combine_subrd($maparray, $type, $main_location, $sub_location,$input_obj,$current_location)
    {
        ...
        if(isset($getfilter->type_view) && $getfilter->type_view!='')
            $str .=' and subrd_type in ('.$getfilter->type_view.')';
        if(isset($getfilter->filter_priority) && $getfilter->filter_priority!='')
             $str .=' and a.subrd_priority="'.$getfilter->filter_priority.'"';
        


        $sql="SELECT  a.`refid`, a.`cluster_id`, a.`cluster_name`, a.`state_name`, a.`district_name`, a.`taluk_name`, a.`village_name`, a.`sector`, a.`loc7`, a.`loc9`, a.`loc10`, a.`loc13`, a.`loc12`, a.`market_id`, a.`bi_id`, a.`distance_subrd`, a.`subrd_loaction`, a.`outlet_potential`, a.`population`, a.`taluk_census`, a.`village_census`, a.`village_choc_consmptn`, a.`cluster_tag`, a.`stat`, a.`subrd_type`, a.`is_hub`, a.`hub_id`, a.`subrd_priority`, a.`tsm_id`, a.`village_2011_census`, a.`company_service_id`,a.exist_subrd_code,a.exist_subrd_name,if(a.company_service_id=1,'Active',if(a.company_service_id=2,'Initd',if(a.company_service_id=3,'Inactive',if(a.company_service_id=4,'Activtd',if(a.company_service_id=5,'Deactivtd',''))))) company_servcng,b.latitude,b.longitude FROM `subrd_data` as a,town_village_polygon as b  where a.village_census=b.town_village_code and a.taluk_census=b.taluk_code and a.tsm_id=".$userid."  ".$str." and  a.stat='A' and b.stat='A' ";
         ...
         for($k=0;$k<$result_count;$k++)
         {
            ...
            unset($temp['child']);
             ...
             $cluster_hub=($final_result[$k]['subrd_type']==1) ? 'SubRD' :(($final_result[$k]['subrd_type']==2) ? 'SubRD' :(($final_result[$k]['subrd_type']==3) ?'Wholesaler' : ''));
             ...
             $temp['subrd_marker']=(($final_result[$k]['subrd_type']==2) ? $priority[$final_result[$k]['subrd_priority']] : (($final_result[$k]['subrd_type']==1) ? 'rural_icon/efficient-subrd.png' : (($final_result[$k]['subrd_type']==3) ? 'rural_icon/Wholesale.png' : 'NA')));
         ...
         $temp['info'] .='<p><span style="color:rgb(242, 101, 34)">Market UID: </span><span style="background-color:white;color:black;">'.$final_result[$k]['market_id'].'</span></p>';
             $temp['info'] .='<p><span style="color:rgb(242, 101, 34)">BI Location ID: </span><span style="background-color:white;color:black;" >'.$final_result[$k]['bi_id'].' </span></p>';
             if(in_array($final_result[$k]['subrd_type'],[1]))
                $temp['info'] .='<p><span style="color:rgb(242, 101, 34)">Exist Subrd Code: </span><span style="background-color:white;color:black;" >'.$final_result[$k]['exist_subrd_code'].' </span></p><p><span style="color:rgb(242, 101, 34)">Exist Subrd Name: </span><span style="background-color:white;color:black;" >'.$final_result[$k]['exist_subrd_name'].' </span></p>';

             $temp['info'] .='<div class="popup-footer" ><span style="background-color:none;text-align:right;" class="navigate_location" onclick="view_village_detail('.$detail.')">More Info</span></div></div>';
            ...
            foreach($final_result[$k]['child'] as $key=>$value)
            {
                 ...
                 $cluster_tag=($value['subrd_type']==1) ? 'SubRD Existing' :(($value['subrd_type']==2) ? 'Subrd Reco' :(($value['subrd_type']==3) ?'Wholesaler' : ''));
                  $cluster_hub=($value['subrd_type']==1) ? 'SubRD' :(($value['subrd_type']==2) ? 'SubRD' :(($value['subrd_type']==3) ?'Wholesaler' : ''));
                  if(isset($maparray[$value['village_census']]))
                {

                       $temp2['info']='<div class="container-fluid pb-1" style="height:fit-content;color:white !important;"><span class="d-flex flex-row  justify-content-between pt-2"><h5>'.$maparray[$value['village_census']]['location_name'].' Villg.&nbsp;</h5><span class="" style="height:max-content;width: 1.3rem; background-color: #00CCCC;border-radius: 50%;text-align: center;" geocode="'.$value['latitude'].','.$value['longitude'].'" onclick="location_navigate(this)"><i class="fa fa-location-arrow" aria-hidden="true" style="font-size:17px;color:black;"></i></span></span><hr style="border-top: 1px solid white;"><p><span style="color:#00CCCC">Recommendation: </span>'.$cluster_type.'</p><p><span style="color:rgb(242, 101, 34)">Distance from Recommd '.$cluster_hub.' (km): </span>'.$value['distance_subrd'].' kms</p><p><span style="color:rgb(242, 101, 34)">Population (2021): </span>'.number_format($value['population'],0).'</p><p><span style="color:rgb(242, 101, 34)">Outlet Potential: </span>'.$value['outlet_potential'].' Nos.</p><p><span style="color:rgb(242, 101, 34)">Villg Choc Consumptn (Annual)(Rs.): </span>'.$value['village_choc_consmptn'].' </p><p><span style="color:rgb(242, 101, 34)">Cluster Tag: </span>'.$cluster_tag.' </p>';
           if(in_array($value['subrd_type'],[2,3]))                
             $temp2['info'] .='<p><span style="color:rgb(242, 101, 34)">SubD Priority: </span> '.$value['subrd_priority'].'</p><p><span style="color:rgb(242, 101, 34)">SubD Cluster Priority: </span>'.$value['subrd_priority'].'</p><p><span style="color:rgb(242, 101, 34)">Market UID: </span><span style="background-color:white;color:black;">'.$value['market_id'].'</span></p>';

             $temp2['info'] .='<p><span style="color:rgb(242, 101, 34)">BI Location ID: </span><span style="background-color:white;color:black;" >'.$value['bi_id'].' </span></p>';
              if(in_array($value['subrd_type'],[1]))
                $temp2['info'] .='<p><span style="color:rgb(242, 101, 34)">Exist Subrd Code: </span><span style="background-color:white;color:black;" >'.$final_result[$k]['exist_subrd_code'].' </span></p><p><span style="color:rgb(242, 101, 34)">Exist Subrd Name: </span><span style="background-color:white;color:black;" >'.$final_result[$k]['exist_subrd_name'].' </span></p>';

             $temp2['info'] .='<div class="popup-footer" ><span style="background-color:none;text-align:right;" class="navigate_location" onclick="view_village_detail('.$value_json.')">More Info</span></div></div>';
           ...
           array_push($column, array(
            ...
            array_push($column, array(
             ...
             'title' => 'Village Choc Consmptn', 'className' => 'text-right'
              ...
              array_push($column, array(
             'title' => 'SubD Priority', 'className' => 'text-left'
         ));
        ...
        for($i=0;$i<count($data);$i++)
        {
            ...
            $temp=array(
                ...
                '<a href="#"  id="'.$data[$i]['child'].'" class="getchild_'.($i+1).'">'. $data[$i]['district_name'] .'</a>',
                 $data[$i]['taluk_name'],
                 '<a href="#" id="' . $data[$i]['village_census'] . '" style="text-decoration:underline" onClick="showbound(this)">' . $data[$i]['village_name'] . '</a>',
                // '<a href="#" style="text-decoration:underline;" onClick="view_village_detail('.$detail.')">'.$data[$i]['village_name'].'</a>',
                  ...
                  '<a href="#" id="' . $data[$i]['subrd_priority'] . '" district="'.$data[$i]['loc9'].'" taluk="'.$data[$i]['taluk_census'].'" subrd_type="' . $data[$i]['subrd_type'] . '" style="text-decoration:underline" onClick="show_priority(this)">' . $data[$i]['subrd_priority'] . '</a>',

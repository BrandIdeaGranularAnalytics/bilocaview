...
class DashboardController extends Controller
{
        ...
        if (
            ...
            if ($user->client_id == 100 || $user->client_id == 130) {
                ...
                $subrd_beat = DB::table("subrd_outlet")                    
                    ->select("subrd_outlet.subrd_id as refid", "subrd_outlet.subrd_name as subrd_name", "subrd_outlet.subrd_code as subrd_code")
                ...
                $district_list = DB::table("subrd_data")
                    ...
                    ->get()
                    ->toArray();
                    // $district_list_wholesale = DB::table("subrd_data")
                    // ->where([["tsm_id", $user->id],["subrd_type",3]])
                    // ->select("loc9 as id", "district_name as district")
                    // ->orderBy("district_name")
                    // ->distinct()
                    // ->get()
                ...
                $taluk_list_ = DB::table("subrd_data")
                    ...
                    ->get()
                    ->toArray();
                    //  $taluk_list_wholesale = DB::table("subrd_data")
                    // ->where([["tsm_id", $user->id],["taluk_census","!=","00000"],["subrd_type",3]])
                    // ->select("taluk_census as id", "taluk_name as taluk", "loc9 as dist_id", "district_name as district")
                    // ->orderBy("taluk_name")
                    // ->distinct()
                    // ->get()
                    ...
                    //  for($i=0;$i<count($taluk_list_wholesale);$i++)
                    // {
                       
                    //     if(!array_key_exists($taluk_list_wholesale[$i]->dist_id, $taluk_list_whole))
                    //     {
                    //         $taluk_list_whole[$taluk_list_wholesale[$i]->dist_id]=[];
                    //         $taluk_list_whole[$taluk_list_wholesale[$i]->dist_id]['district']=$taluk_list_wholesale[$i]->district;
                    //         $taluk_list_whole[$taluk_list_wholesale[$i]->dist_id]['id']=$taluk_list_wholesale[$i]->dist_id;
                    //         $taluk_list_whole[$taluk_list_wholesale[$i]->dist_id]['taluk_list']=[];
                    //         $temp=[];
                    //           $temp['id']=$taluk_list_wholesale[$i]->id;
                    //           $temp['taluk']=$taluk_list_wholesale[$i]->taluk;
                    //           array_push($taluk_list_whole[$taluk_list_wholesale[$i]->dist_id]['taluk_list'],$temp);
    ...
    public function getsubrdbeat($input)
    {
         ...
         array_push($column, array(
          ...
          array_push($column, array(
             'title' => 'Sub RD Market-UID', 'className' => 'text-right'
         ));
           array_push($column, array(
             'title' => 'Sub Distributor code', 'className' => 'text-right'
         ));
          array_push($column, array(
             'title' => 'Sub Distributor Name', 'className' => 'text-left'
         ));
           array_push($column, array(
             'title' => 'SubD District', 'className' => 'text-left'
         ));
           array_push($column, array(
             'title' => 'SubD Taluk', 'className' => 'text-left'
         ));
            array_push($column, array(
             'title' => 'SubD Village', 'className' => 'text-left'
         ));
            array_push($column, array(
            ...
            array_push($column, array(
             ...
             'title' => 'Village Name', 'className' => 'text-left'
                  ...
                  array_push($column, array(
             'title' => 'Village Taluk', 'className' => 'text-left'
         ));
                  array_push($column, array(
             'title' => 'Village District', 'className' => 'text-left'
         ));
                  
                     array_push($column, array(
             'title' => 'Village State', 'className' => 'text-left'
         ));
        ...
        array_push($column, array(
         ...
         array_push($column, array(
            ...
            array_push($column, array(
             ...
             array_push($column, array(
             'title' => 'No. of Visi Cooler Outlets Per Beat', 'className' => 'text-right'
         )); 
         array_push($column, array(
             'title' => 'SubD Type', 'className' => 'text-left'
         ));             
      ...
      // echo $sql;die;
        ...
        $test=['No Visit'=>3,'Premium'=>0,'Medium'=>1,'Van Beat'=>2];

         $subrd_arr=range(0,30);$colorval=0;$premium=[];
        for($s=0;$s<$total_potential;$s++)
        {
             // Sub RD Market-UID   Sub Distributor code    State   Sub Distributor Name    SubD District   SubD Taluk  SubD Village    SubD Latitude   SubD Longitude  Beat          Village Market-UID  Village Name    Village District    Village Taluk   Village State   One Way Distance (Kms) Between Villages One Way Travelling Time between villages (Mins) One Way Distance Covered (Kms) per Beat No. of Outlets in each Village  Total Outlets Covered per Beat  No. of wholesalers in each Village  No. of visicoolers in each Village  No. of Wholesalers per Beat No. of Visi Cooler Outlets Per Beat Assumed Overall Time (Hrs)     Mondelez Classification 


            $val_data=array(($s+1),$res[$s]['subrd_id'],$res[$s]['subrd_code'],'<a href="#" style="text-decoration:underline;" onClick="show_subrdbeat(0,'.$res[$s]['subrd_id'].')">'.ucwords(strtolower($res[$s]['subrd_name'])).'</a>',$res[$s]['subrd_district'],$res[$s]['subrd_taluk'],$res[$s]['village_name'],'<a href="#" style="text-decoration:underline;" onClick="show_subrdbeat('.$res[$s]['beat_id'].',0)">'.$res[$s]['beat_name'].'</a>',$res[$s]['beat_unique_id'],$res[$s]['village_market_id'],$res[$s]['visit_order'],$res[$s]['village_name'],$res[$s]['village_taluk'],$res[$s]['village_district'],$res[$s]['village_state'],$res[$s]['oneway_distance'],$res[$s]['beatween_distance'],$res[$s]['oneway_distance_per_beat'],$res[$s]['covered_outlets'],$res[$s]['covered_village'],$res[$s]['covered_wholesaler'],$res[$s]['covered_visicooler'],$res[$s]['beat_wholesaler'],$res[$s]['covered_cooler_outlets'],$type_subrd[$res[$s]['subrd_type']],$res[$s]['overall_time'],$res[$s]['premium']);
            ...
            if(!in_array($res[$s]['subrd_id'], $subrd_id))
            {
                ...
                $temp=[];
                 ...
                 $temp['info']='<div class="container-fluid pb-2" style="height:fit-content;color:white !important;"><span class="d-flex flex-row  justify-content-between pt-2"><h5>'.$temp['subrd_name'].'</h5><span class="" style="height:max-content;width: 1.3rem;background-color: #00CCCC;border-radius: 50%;text-align: center;" geocode="'.$res[$s]['subrd_latitude'].','.$res[$s]['subrd_longitude'].'" onclick="location_navigate(this)"><i class="fa fa-location-arrow" aria-hidden="true" style="font-size:17px;color:black;"></i></span></span><hr style="border-top: 1px solid white;"><p><span style="color:rgb(242, 101, 34)">Subrd Code: </span>'.$res[$s]['subrd_code'].'</p><p><span style="color:rgb(242, 101, 34)">Subrd Type: </span>'.$temp['type'].' </p><p><span style="color:rgb(242, 101, 34)">State: </span>'.$res[$s]['village_state'].'</p><p><span style="color:rgb(242, 101, 34)">District : </span>'.$res[$s]['subrd_district'].' </p><p><span style="color:rgb(242, 101, 34)">Taluk: </span>'.$res[$s]['subrd_taluk'].' </p></div>';
                ...
                $temp['info']='<div class="container-fluid pb-2" style="height:fit-content;color:white !important;"><span class="d-flex flex-row  justify-content-between pt-2"><h5>'.$res[$s]['village_name'].'</h5><span class="" style="height:max-content;width: 1.3rem;background-color: #00CCCC;border-radius: 50%;text-align: center;" geocode="'.$res[$s]['village_latitude'].','.$res[$s]['village_longitude'].'" onclick="location_navigate(this)"><i class="fa fa-location-arrow" aria-hidden="true" style="font-size:17px;color:black;"></i></span></span><hr style="border-top: 1px solid white;"><p class="text-wrap"><p><span style="color:rgb(242, 101, 34)">Beat Name : </span>'.$res[$s]['beat_name'].' </p><p><span style="color:rgb(242, 101, 34)">Village Market ID : </span>'.$res[$s]['village_market_id'].' </p><p><span style="color:rgb(242, 101, 34)">Visit Order : </span>'.$res[$s]['visit_order'].' </p><p><span style="color:rgb(242, 101, 34)">Covered Outlets : </span>'.$res[$s]['covered_outlets'].' </p><p><span style="color:rgb(242, 101, 34)">Covered VisiColler: </span>'.$res[$s]['covered_visicooler'].' </p><p><span style="color:rgb(242, 101, 34)">Covered Wholesaler: </span>'.$res[$s]['covered_wholesaler'].' </p><p><span style="color:rgb(242, 101, 34)">State: </span>'.$res[$s]['village_state'].'</p><p><span style="color:rgb(242, 101, 34)">District: </span>'.$res[$s]['village_district'].' </p><p><span style="color:rgb(242, 101, 34)">Taluk: </span>'.$res[$s]['village_taluk'].'</p></div>';
     ...
     public function gethighway($input)
     {
         ...
         array_push($column, array(
           ...
           array_push($column, array(
             'title' => 'Recommended SubRd Loc Market ID', 'className' => 'text-right'
         ));
            ...
            if(!in_array($res[$s]['subrd_id'], $subrd_id))
            {
                array_push($subrd_id,$res[$s]['subrd_id']);
                 ...
                 //    $temp['info']='<div class="tooltip-data"><div class="card"><div class="card-header"><h3>'.$subrd_name.'</h3> </div>'.$res[$s]['sub_text'].'</div></div>';
                     ...
                     $temp['info']='<div class="container-fluid pb-2" style="height:fit-content;color:white !important;"><span class="d-flex flex-row  justify-content-between pt-2"><h5>'.$subrd_name.'</h5><span class="" style="height:max-content;width: 1.3rem;background-color: #00CCCC;border-radius: 50%;text-align: center;" geocode="'.$res[$s]['subrd_lat'].','.$res[$s]['subrd_lon'].'" onclick="location_navigate(this)"><i class="fa fa-location-arrow" aria-hidden="true" style="font-size:17px;color:black;"></i></span></span><hr style="border-top: 1px solid white;"><p><span style="color:rgb(242, 101, 34)">Subrd Code: </span>'.$res[$s]['subrd_code'].'</p><p><span style="color:rgb(242, 101, 34)">Subrd Type: </span>'.$res[$s]['subrd_name_type'].' </p><p><span style="color:rgb(242, 101, 34)">State: </span>'.$res[$s]['state'].'</p><p><span style="color:rgb(242, 101, 34)">District : </span>'.$res[$s]['district'].' </p><p><span style="color:rgb(242, 101, 34)">Taluk: </span>'.$res[$s]['taluk'].' </p></div>';
                ...
                $temp['info']='<div class="container-fluid pb-2" style="height:fit-content;color:white !important;"><span class="d-flex flex-row  justify-content-between pt-2"><h5>'.$res[$s]['ccp_name'].'</h5><span class="" style="height:max-content;width: 1.3rem;background-color: #00CCCC;border-radius: 50%;text-align: center;" geocode="'.$res[$s]['latitude'].','.$res[$s]['longitude'].'" onclick="location_navigate(this)"><i class="fa fa-location-arrow" aria-hidden="true" style="font-size:17px;color:black;"></i></span></span><hr style="border-top: 1px solid white;"><p class="text-wrap"><span style="color:#00CCCC">'.$res[$s]['address'].'</p><p><span style="color:rgb(242, 101, 34)">Channel: </span>'.$res[$s]['channel'].'</p><p><span style="color:rgb(242, 101, 34)">Subrd Code: </span>'.$res[$s]['subrd_code'].' </p><p><span style="color:rgb(242, 101, 34)">Subrd Name: </span>'.$subrd_name_.'</p><p><span style="color:rgb(242, 101, 34)">State : </span>'.$res[$s]['state'].' </p><p><span style="color:rgb(242, 101, 34)">District: </span>'.$res[$s]['district'].' </p><p><span style="color:rgb(242, 101, 34)">Taluk: </span>'.$res[$s]['taluk'].' </p><p><span style="color:rgb(242, 101, 34)">Village: </span>'.$res[$s]['taluk'].' </p></div>';

    ...
    public function show_outletlist_bycategory($maparray,$type,$main_location,$sub_location,$so_id,$input_obj,$current_location)
    {
        ...
        if($type==9)
        {
           ...
           ->select('id as refid'  , 'secondary_channel_type as channel', 'secondary_channel_type as subchannel', 'name as outlet_name', 'address as address', 'latitude as lat', 'longitude as lon')
            ...
            $filterchannel='';$filterpotential='';
            if(isset($obj['filter_bychannel']) && $obj['filter_bychannel']!='' && $obj['filter_bychannel']!=0)
              $filterchannel=' and maintype_id in ("'.$obj['filter_bychannel'].'")';
           if(isset($obj['filter_bypotential']) && $obj['filter_bypotential']!='' && $obj['filter_bypotential']!=0)
              $filterpotential=' and Estimtd_potntl in ("'.$obj['filter_bypotential'].'")';




          // $data_uncovered_outlet_list = "SELECT refid,rtlr_id,main_type as channel,SubType as subchannel,ccp as outlet_name,address as address,latitude as lat,longitude as lon,status,maintype_icon,maintype_id, (((acos(sin((".$lat."*pi()/180)) * sin((`latitude`*pi()/180)) + cos((".$lat."*pi()/180)) * cos((`latitude`*pi()/180)) * cos(((".$lon."- `longitude`) * pi()/180)))) * 180/pi()) * 60 * 1.1515 * 1.609344) as distance FROM uncovered_outlets where salesman_id='".$userid."' and latitude!='' ".$filterchannel." ".$filterpotential." order by distance asc";
           $data_uncovered_outlet_list = "SELECT refid,rtlr_id,main_type as channel,SubType as subchannel,ccp as outlet_name,address as address,latitude as lat,longitude as lon,status,maintype_icon,maintype_id FROM uncovered_outlets where salesman_id='".$userid."' and latitude!='' ".$filterchannel." ".$filterpotential."";
       ...
       else
       {
        ...
        for($i=0;$i<count($data_outlet_list);$i++)        
        {
             ...
             {
                $temp['icon']= ($data_outlet_list[$i]->status=='N') ? $data_outlet_list[$i]->maintype_icon : (($data_outlet_list[$i]->status=='A') ? 'images/covered.png' : 'images/nr.png');                
                $temp['type']='uncovered';
             }
             else
             {
        ...
        if($type==9)
        {
           ...
           for($i=0;$i<count($data_uncovered_outlet_list);$i++)        
          {
               ...
               $temp['address']=ucwords(strtolower($data_uncovered_outlet_list[$i]->address));
                $temp['icon']= ($data_uncovered_outlet_list[$i]->status=='N') ? $data_uncovered_outlet_list[$i]->maintype_icon : (($data_uncovered_outlet_list[$i]->status=='A') ? 'images/covered.png' : 'images/nr.png'); 

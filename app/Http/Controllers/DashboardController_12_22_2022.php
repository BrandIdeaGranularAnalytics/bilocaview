...
class DashboardController extends Controller
{
    ...
    public function loadmapPost(Request $request)
    {
        ...
        if(!isset($getfilter->filter_highway))
        {
     ...
     public function gethighway($input)
     {
        ...
        for($s=0;$s<$total_potential;$s++)
        {
            ...
            if(!array_key_exists($res[$s]['highway_id'], $highway['highway_list']))
            {
                 ...
                 $highway['highway_list'][$res[$s]['highway_id']]['info']='<div class="tooltip-data popupdata"><div class="card"><div class="card-header"><h3>'.$res[$s]['highway_name'].'</h3></div><ul class="list-group list-group-flush"><li>Stretch Length:<span>'.$res[$s]['length'].' Km.</span</li><li>Start Point:<span>'.$res[$s]['start_point'].'</span</li><li>End Point:<span>'.$res[$s]['end_point'].'</span</li><li>Outlet Potential:<span>'.$highway['highway_list'][$res[$s]['highway_id']]['highway_potential'].' Nos.</span</li></ul></div></div>';
            ...
            if(!in_array($res[$s]['subrd_id'], $subrd_id))
            {
                array_push($subrd_id,$res[$s]['subrd_id']);
                 ...
                 $res[$s]['sub_text']=  '<ul class="list-group list-group-flush"><li class="list-group-item" >Subrd Code:<span>'.$res[$s]['subrd_code'].'</span></li><li class="list-group-item" >Subrd Type:<span>'.$res[$s]['subrd_name_type'].'</span></li><li class="list-group-item" >'.$res[$s]['subrd_address'].'</li><li class="list-group-item" >State:<span>'.$res[$s]['state'].'</span></li><li class="list-group-item" >District:<span>'.$res[$s]['district'].'</span></li><li class="list-group-item" >Taluk:<span>'.$res[$s]['taluk'].'</span></li></ul>' ;
                ...
                $temp['info']='<div class="tooltip-data"><div class="card"><div class="card-header"><h3>'.$res[$s]['ccp_name'].'</h3> </div><ul class="list-group list-group-flush"><li class="list-group-item">'.$res[$s]['address'].'</li><li class="list-group-item">Channel <span>'.$res[$s]['channel'].'</span></li><li class="list-group-item">Subrd Code <span>'.$res[$s]['subrd_code'].'</span></li><li class="list-group-item">Subrd Name <span>'.$subrd_name_.'</span></li><li class="list-group-item">Group <span>'.$res[$s]['group_name'].'</span></li><li class="list-group-item">State: <span>'.$res[$s]['state'].'</span></li><li class="list-group-item">District: <span>'.$res[$s]['district'].'</span></li><li class="list-group-item">Taluk: <span>'.$res[$s]['taluk'].'</span></li></ul></div></div>';

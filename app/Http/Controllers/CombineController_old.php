        ...
        for ($i = 0;$i < count($data);$i++)
        {
          ...
          if(isset($data[0]['stat']))
            $temp = array(
                ...
                $data[$i]['address'],$data[$i]['stat']

            );
          else
            $temp = array(
                ($i + 1) ,  '<a href="#"  style="text-decoration:underline" onClick="highlight('.$data[$i]['refid'].','.$data[$i]['lat'].','.$data[$i]['lon'].')">'.$data[$i]['outlet_name'].'</a>',
                $data[$i]['channel_name'],
                $data[$i]['sub_channel_name'],
                $data[$i]['address'],
    ...
    public function show_outletlist_bycategory($maparray,$type,$main_location,$sub_location,$so_id)
    {
       ...
       if($type !=8)
       {
          $data_outlet_list =  DB::table('ref_02sep2021')      
            ...
            ->select('fld580 as refid'  ,  'type as channel', 'subtype as subchannel', 'loc16', 'fld1054', 'CCP_Name as outlet_name', 'address as address', 'latitude as lat', 'longitude as lon','icon','shop_image','status')
            ->get();
       }
       if($type ==8)
       {
        ...
        for($i=0;$i<count($data_outlet_list);$i++)        
        {
             ...
             $temp['stat']='';
             if(isset($data_outlet_list[$i]->status))
             {
                if($data_outlet_list[$i]->status==1)
                  $temp['stat']='Found';
                 if($data_outlet_list[$i]->status==2)
                  $temp['stat']='Not Found';

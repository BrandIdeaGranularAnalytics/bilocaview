...
class CombineController extends Controller
{
               ...
               // $data[$i]['channel_name'],
                ...
                // $data[$i]['lon'],
                 ((isset($data[$i]['maintype_id'])) ?  (($data[$i]['status']=='R') ? 'Not Relevent' : (($data[$i]['status']=='A') ? 'Activated' : 'Uncovered')) : 
                  (($data[$i]['status']=='R') ? 'Not Found' : (($data[$i]['status']=='A') ? 'Found' : (($data[$i]['status']=='V') ?  'Visited' : 'Not Visited' )))
                )
    ...
    public function show_outletlist_bycategory($maparray,$type,$main_location,$sub_location,$so_id,$input_obj)
    {
        $data = [];
        $data['result'] = array();      
